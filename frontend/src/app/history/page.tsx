...
export default async function HistoryPage() {
  ...
  const rows = db
    ...
    .all(session.email) as unknown as ConversionRow[];
    ...
    <main className="flex-1 bg-slate-50 p-4 sm:p-8">
      <div className="max-w-5xl mx-auto">
        <div className="flex items-center gap-3 mb-6">
          ...
          <div>
            <h1 className="text-xl font-bold text-slate-800">Your conversion history</h1>
            <p className="text-sm text-slate-500">
              Files you've cleaned, with re-download links. Stored copies are kept for 30 days.
            </p>
          </div>
        </div>
        ...
        <div className="bg-white border border-slate-200 rounded-2xl shadow-sm overflow-hidden">
          {rows.length === 0 ? (
            <p className="p-8 text-center text-sm text-slate-500">
              You haven't run any conversions yet.
            </p>
          ) : (
